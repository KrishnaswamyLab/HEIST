...
def eval_roc_auc(model, loader):
    ...
    return roc_auc_score(true, scipy.special.softmax(pred, 1),multi_class='ovo')
...
if __name__ == '__main__':
    print("================================")
    print("Tissue classification Placenta Condition")
    print("================================")
    _high_level_graphs = torch.load(f"data/yang_hartwich_{'_'.join(args.model_name.split('_')[1:])}.pt")
    labels = torch.LongTensor([getattr(i, 'label') for i in _high_level_graphs]).to(args.device)
    ...
    for fold in range(5):
        model = MLP(embeddings.shape[1], args.hidden_dim, 3, args.num_layers).to(args.device)
        ...
        val_idx, test_idx = train_test_split(test_idx.cpu().numpy(), test_size = 0.5, stratify= labels[test_idx].cpu().numpy())
